            ...
            if ($emailCheck->rowCount() <= 0) {
               ...
               if ($userCheck->rowCount() <= 0) {
                   ...
                   $passwordResetCode = crypt($password, "G0dASh0ftH3W0r1d");
                    ...
                    $userResult = $db->prepare("insert into users (username, password, passwordResetCode, email, firstname, lastname, create_datetime, edit_datetime) values ('{$username}', '{$cryptPass}', '{$passwordResetCode}', '{$email}', '{$firstname}', '{$lastname}', NOW(), NOW())");
                    $userResult->execute();
